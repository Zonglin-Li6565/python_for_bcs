"""
Remember numpy? It's the python module for dealing with large data arrays and
matrix data.
...
"""
what is this "as np" business? This just means we can refer to numpy as np 
instead of as numpy every time we use it.
it shortens it and makes it easier to type.
"""
some_list = [1, 2, 3]
some_array = np.array(
    some_list)  # this command changes a python list into a numpy array
"""
We could use "as" to change a module's name to anything we want.
I could "import numpy as pizza", and then replace the line above with "x = 
pizza.array(some_list)"
But that wouldnt be shorter or faster to type...

Let's prove that numpy actually goes faster than core python to do math.
Say I want to compute the mean of a list/array, and the dot product of two 
lists/arrays.
...
# how much slower was this on your machine compared to the core python loop
# method?
...
"""
And of course, numpy let's us do all kinds of math magic with arrays, 
avoiding those annoying loops.
Say I want to compute dot products of two arrays (the sum of the product of 
each element:
...
"""
Another advantage of numpy is that we can store a bunch of data in a matrix, 
instead of as lists of lists, which is really annoying, if you will recall.
"""
# comment this line
x_matrix = np.random.normal(100, 10, [1000, 2])
print(x_matrix)
print()
# note that numpy doesnt print out all the data by default, but summarizes it
# using ...

# we can compute statistics over the whole data matrix, or on rows or columns
# separately.
...
mean1 = x_matrix.mean()
# The mean of axis 0 which is the mean of each column
mean2 = x_matrix.mean(0)
# The mean of axis 1 which is the mean of each row
mean3 = x_matrix.mean(1)
# The standard deviation of the whole matrix
stdev1 = x_matrix.std()
